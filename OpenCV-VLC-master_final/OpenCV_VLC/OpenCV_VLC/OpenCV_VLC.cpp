...
int main()
{
	...
	hThread = (HANDLE)_beginthreadex(NULL, 0, HandleClnt, (void *)context, 0, NULL);
...
unsigned WINAPI HandleClnt(void * arg) { // 
	...
	while (1)
	{
		...
		if (((int)duration == 1))   // 1 = 1초마다 영상을 분석함
		{
			...
			Mat frame_f = ctx->image->clone();
			Mat frame_gray;
			cv::cvtColor(frame_f, frame_gray, COLOR_BGR2GRAY);
			...
			for (size_t i = 0; i < faces.size(); i++)
			{
				...
				cv::rectangle(frame_f, lb, tr, cv::Scalar(0, 255, 0), 3, 4, 0);
