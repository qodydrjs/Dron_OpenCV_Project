...
unsigned WINAPI SendImage(void *arg);
		...
		if (!(clntCnt <= 0)  && duration_SendImage > 1)
		{
			...
			for (i = 0; i < clntCnt; i++) {
				...
				try
				{
					...
					send(clntSocks[i], buf_start, sizeof(buf_start), 0);//이미지 시작 Flag 전송
					send(clntSocks[i], c_size, str_size.length(), 0); //이미지 사이즈 전송
					...
					while (qwFileSize > 0) {
						...
						retval = send(clntSocks[i], file.pbFile, dwBytesInBlock, 0);
					...
					send(clntSocks[i], buf_end, sizeof(buf_end), 0);//이미지 시작 Flag 전송
